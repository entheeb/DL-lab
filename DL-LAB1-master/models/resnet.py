...
class Resblock_next(k.layers.Layer):
    def __init__(self,
                 num_channel,
                 first_block=False,
                 Head=False,
        ...
        super(Resblock_next, self).__init__(name=name)
        self.Head = Head
        stride = 1
        if Head and not first_block:
            stride = 2
        self.conv1 = [k.layers.Conv2D(filters=num_channel / 16,
                                      kernel_size=1,
                                      padding='same',
                                      strides=stride,
                                      kernel_initializer=initializer,
                                      kernel_regularizer=regularizer) for _ in range(32)]

        self.conv2 = [k.layers.Conv2D(filters=num_channel / 16,
                                      kernel_size=3,
                                      padding='same',
                                      kernel_initializer=initializer,
                                      kernel_regularizer=regularizer) for _ in range(32)]
        ...
        if Head or first_block:
            self.conv_bypass = k.layers.Conv2D(filters=4 * num_channel,
                                               ...
                                               kernel_initializer=initializer,
                                               kernel_regularizer=regularizer)
        self.bn1 = [k.layers.BatchNormalization() for _ in range(32)]
        self.bn2 = [k.layers.BatchNormalization() for _ in range(32)]
        self.bn3 = [k.layers.BatchNormalization() for _ in range(32)]

    def call(self, inputs):
        outputs = []
        for i in range(32):
            output = tf.nn.relu(self.bn1[i](self.conv1[i](inputs)))
            output = tf.nn.relu(self.bn2[i](self.conv2[i](output)))
            output = self.bn3[i](self.conv3[i](output))
