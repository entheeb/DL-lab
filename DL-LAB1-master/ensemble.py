    ...
    for ckpt_fn in ckpt_fns:
        for model_id, model_name in Model_index.items():
            if model_name in ckpt_fn:
                model, model_info = choose_model(model_id=model_id, ckpt_fn=ckpt_fn, Mode='Evaluate', run_paths=run_paths)
                model.trainable = False
                model_stack.append(model)
    return model_stack, model_info
...
@gin.configurable
def Voting(test_ds, num_classes, run_paths, ckpt_fns):
    ...
    models, _ = load_models(ckpt_fns, run_paths) if isinstance(ckpt_fns, list)\
    ...
    tf.print(tf.squeeze(pred) for pred in preds)
