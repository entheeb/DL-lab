...
def Ensemble(run_paths):
    ...
    Voting(test_ds, run_paths=run_paths)
...
def Dataset(run_paths, model_info=None):
    if model_info is None:
        model_info = {'input_shape': (256, 256, 3), 'transfer': False}
    ...
    train_ds = build_dataset_from_tfrecord(train_files, train_labels, run_paths, model_info, "Train")
    validation_ds = build_dataset_from_tfrecord(valid_files, valid_labels, run_paths, model_info, "Validation",
                                                test_set=True)
    test_ds = build_dataset_from_tfrecord(test_files, test_labels, run_paths, model_info, "Test", test_set=True)
